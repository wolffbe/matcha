...
def create_pattern_image(seed=42, width=640, height=480):
    ...
    # Start with random noise base (lots of high-frequency detail)
    ...
    # Add random lines (creates edges that pHash is sensitive to)
    for _ in range(50):
    ...
    # Add some shapes with varying transparency effect
    for _ in range(20):
        x1 = np.random.randint(0, width)
        y1 = np.random.randint(0, height)
        x2 = np.random.randint(0, width)
...
def save_image(arr, filename, fmt="PNG"):
    """Save image array to tmp directory."""
...
def query_match(query_path, image_hamming_distance):
    ...
    with open(query_path, "rb") as f:
        r = requests.post(
            ...
            files={"file": (os.path.basename(query_path), f, "image/png")}
        )
    ...
    if isinstance(matches, dict):
        # Error response or single result as dict
...
def run_calibrations_for_hamming(image_hamming_distance, base_path, base_item_id, different_path):
    ...
    # Brightness tests (level = pct * 2.5)
    for pct in PERCENTAGES:
        expected = pct <= MATCH_THRESHOLD
    ...
    # Contrast tests (factor = 1 + pct * 0.05)
    for pct in PERCENTAGES:
        expected = pct <= MATCH_THRESHOLD
        key = f"contrast_{pct}"
        name = f"Contrast {pct}%"
...
def find_optimal_hamming(min_hamming=1, max_hamming=20):
    ...
    print(f"Calibrating video hamming distances from {min_hamming} to {max_hamming}")
    print("=" * 80)
    print()
    
    # Create and save base image
    print("Creating base image...", end=" ", flush=True)
    base_arr = create_pattern_image(seed=42)
    base_path = save_image(base_arr, "base.png")
    print("done")
    
    # Create and save different image
    ...
    for hamming in range(min_hamming, max_hamming + 1):
        ...
        # Reset and index base image
        reset()
        with open(base_path, "rb") as f:
            hash_response = requests.post(
                f"{BASE_URL}/hash",
                files={"file": ("base.png", f, "image/png")}
            )
        if hash_response.status_code != 200:
            print(f"Hash failed: {hash_response.json()}")
            continue
        
        base_item_id = hash_response.json().get("item_id")
    ...
    return best["image_hamming_distance"], all_results, best, elapsed_time
...
def format_results(stats):
    ...
    # Add cutoff tests - all four sides
    for side in ["right", "left", "top", "bottom"]:
        for pct in PERCENTAGES:
            key = f"cutoff_{side}_{pct}"
...
def format_all_results(all_results):
    """Format all hamming distance results as a string."""
    lines = []
    lines.append("")
    lines.append("=" * 80)
    lines.append("ALL VIDEO HAMMING DISTANCES")
    lines.append("=" * 80)
    lines.append("")
...
def main():
    print("=" * 80)
    print("Finding Optimal Video Hamming Distance (for Images)")
    print("=" * 80)
    print()
