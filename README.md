...
Where exact hashing only recognizes identical files, Matcha recognizes media that has been altered by cropping, trimming, transcoding, pitch shifting, blurring or logo stamping, and still traces it back to the original. This makes it a building block for deduplication, copyright and brand protection, and tracking the spread of known content across platforms.

* **Robust to manipulation**: near matches survive crops, trims, logo overlays, compression, brightness and contrast changes, and pitch shifts that defeat cryptographic hashes.
* **Multimodal by design**: video is matched independently on frames, audio and transcript, so a heavily cropped video still matches through its unchanged soundtrack.
...
* **Ready to operate**: an API with two endpoints (`/hash` and `/match`), thresholds tunable per request, project isolation for multiple tenants, and calibrated defaults backed by an automated test suite.
